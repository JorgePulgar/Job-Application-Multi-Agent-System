...
## Acceptance criteria
- [ ] `src/agents/job_scraper/wttj.py` no longer hits `/es/jobs`. Constants and tests updated.
- [ ] Pipeline returns ≥ 1 real `JobOffer` for at least one user profile's target roles when run against the live site, OR the scraper logs a structured warning and returns `[]` cleanly when WTTJ blocks rendering (no crash, no partial data).
- [ ] At least one of the following strategies is implemented and documented:
  ...
  - **C.** Pivot to a different platform (drop WTTJ from `config/sources.yaml`, document why).
- [ ] If A or B: pure parsing function still consumed via the existing `parse_job_cards` / `cards_to_offers` pipeline (or an Algolia-JSON equivalent) so unit tests remain mockable without live network.
- [ ] Unit tests updated: new HTML/JSON fixture matching the new strategy. Tests must NOT hit the live site.
- [ ] Integration test or manual run captured in commit body: number of offers fetched, runtime, any selector misses.
- [ ] README setup section updated if any new env vars (e.g. Algolia keys) or browser flags are required.

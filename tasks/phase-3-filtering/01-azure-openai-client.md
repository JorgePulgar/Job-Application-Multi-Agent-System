...
## Acceptance criteria
- [x] `src/services/azure_openai.py` exposes `class AzureOpenAIClient` with:
...
- [x] Uses the official `openai` SDK with `AzureOpenAI` / `AsyncAzureOpenAI`, `api_version` from settings.
- [x] Maps `deployment="mini"` → `AZURE_OPENAI_DEPLOYMENT_MINI`, `"4o"` → `AZURE_OPENAI_DEPLOYMENT_4O`.
- [x] Enables prompt caching for stable system content (mark system message as cache-eligible via the SDK's caching mechanism for the API version in use).
- [x] Retries: 3 attempts with exponential backoff on `RateLimitError` and 5xx; no retries on 4xx other than 429.
- [x] When `response_format` is a Pydantic model, uses the SDK's structured outputs feature and returns the parsed object.
- [x] All token usage forwarded to a `usage_tracker` (Phase 7 will plug into this; for now just expose the hook).

...
## Acceptance criteria
- [ ] `config/users/jorge.yaml.example` and `config/users/novia.yaml.example` are realistic skeletons covering every `UserProfile` field. CV summary is in Spanish.
- [ ] Real YAMLs (`jorge.yaml`, `novia.yaml`) are gitignored; the loader expects them at the same path.
...
- `config/users/novia.yaml.example`

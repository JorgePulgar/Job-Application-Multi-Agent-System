...
## Acceptance criteria
- [x] `src/models/user_profile.py` defines:
...
- [x] All models use Pydantic v2 (`model_config = ConfigDict(...)`, validators with `@field_validator` / `@model_validator`).
- [x] `username` must be lowercase, alphanumeric/underscore, 2-32 chars (validator).
- [x] `email` validated as email format.
- [x] `min_salary`, if set, must be a positive int.
- [x] Helpers `UserProfile.from_yaml(path: Path) -> UserProfile` and `UserProfile.cv_for_prompt() -> str` (markdown formatted) exposed.

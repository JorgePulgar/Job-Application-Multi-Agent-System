...
## Acceptance criteria
- [x] `src/graph/nodes/assess_fit.py` → `{"fit": FitAssessment}`. `gpt-4o`,
      `response_format=FitAssessment`.
- [x] Reads `parsed`, `dossier`, `sponsorship`, `requirements` from state.
- [x] Prompt `src/prompts/graph_assess_fit.{system,user}.md` encodes README §6:
...
- [x] Few-shot with 2-3 examples covering apply / maybe / skip (mirrors v1
      evaluator convention). _3 examples (A=apply, B=maybe, C=skip) in system prompt._
- [x] `score` (0-100) consistent with `fit_level`/`recommendation`. _Bands in prompt:
      apply≈70-100, maybe≈40-69, skip≈0-39._
- [x] User-facing text (`reasoning`, `red_flags`, `tailoring`) emitted in
      `parsed.detected_language`. _Language directive in user prompt (cache convention)._

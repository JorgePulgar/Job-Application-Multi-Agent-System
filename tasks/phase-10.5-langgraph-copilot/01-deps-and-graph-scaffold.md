...
## Acceptance criteria
- [ ] **Compatibility check first:** confirm `langgraph` + `langfuse` support the
...
- [ ] `uv add langgraph langfuse` (and `langgraph-checkpoint-sqlite` if the SQLite
      saver is a separate package for the resolved version). Lockfile committed.
- [ ] `src/graph/__init__.py`, `src/graph/state.py`, `src/graph/build.py` created.
- [ ] `src/graph/state.py` defines `EvaluateDraftState` (TypedDict, `total=False`)
      exactly per README §4.
- [ ] `src/graph/build.py` exposes `build_graph(checkpointer) -> Compiledgraph`
      that wires node stubs (each node a `pass`/placeholder returning `{}`) so the
      graph **compiles** and a smoke test can `.get_graph()`/draw it.
- [ ] `mypy --strict src/graph/` passes (add `langgraph`/`langfuse` to mypy

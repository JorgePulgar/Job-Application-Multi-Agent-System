...
## Acceptance criteria
- [ ] `src/services/web_search.py` exposes `async def search_web(query: str, n: int = 10) -> list[SearchResult]` where `SearchResult` is `{title, url, snippet}` (Pydantic model).
- [ ] Provider selection: if `BING_SEARCH_KEY` is set, use Bing; else use `duckduckgo-search`. The chosen provider is logged once on startup.
- [ ] Polite rate limiting (max 5 req/s for Bing, 1 req/s for DDG).
- [ ] Returns at most `n` results; trims and dedupes by URL.
- [ ] No exceptions leak — failed search returns `[]` and logs a warning.
